...
<div class="row">
	<div class="col-md-3">
		...
		<button type="button" class="btn btn-primary shadow btn-block" data-toggle="modal" data-target="#tambah">Tambah Kriteria</button>
	</div>
	<div class="col-md-9">
		<?php if ($this->session->flashdata('notif')) { ?> 
			...
			<!-- Menampilkan Notif -->
		<?php }else{ ?>
			<div class="alert alert-warning shadow alert-icon mb-3" role="alert">
				<b>Klik pada value/item yang ingin dirubah/diedit</b>
			</div>
		<?php }?>
		<br>
		<div class="card shadow mb-3">
			...
			<div class="card-body">
					<table id="dataTable" class="table table-bordered table-sm table-hover">
						<thead>
							<tr>
								<th width='4%'>No</th>
								<th width='34%'>Kriteria</th>
								<th width='40%'>Keterangan</th>
								<th width='17%' class="text-center">Bobot (%)</th>
								<th width='5%'>AKSI</th>
							</tr>
						</thead>
						<tbody>
       ...
       // User List
							foreach($get_kriteria as $data){
								...
								echo "<td>
								<span class='edit text-center' data-toggle='tooltip' data-placement='top' title='Klik untuk mengubah'>".$data->bobot."</span>
			...
			<div class="modal-content">
				...
				<form action="<?php echo site_url('penilaian/proses_make/'.$id_tahap.'/'.$id_lomba);?>" method="POST">
					...
					<div class="modal-footer">
						...
						<button type="button" name="add" id="adds" class="btn btn-info btn-sm">Tambahkan Field</button>
					</div>
				</form>
			</div>
		</div>
	...
	<script>  
		$(document).ready(function(){  
			...
			$('#adds').click(function(){  
				...
				$('#dynamic_field').append('<tr id="row'+i+'"><td width="40%"><input type="file" name="kriteria[]" placeholder="Kriteria '+i+'" class="form-control name_list mb-3" /><input type="number" name="bobot[]" placeholder="Bobot %" class="form-control name_list" /><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn-sm btn-block mt-3 btn_remove"><span class="icon text-white-50"><i class="fas fa-trash"></i> Hilangkan Kolom ini</button></td><td width="60%"><textarea type="text" name="keterangan[]" placeholder="Keterangan" rows="3" class="form-control name_list" /></textarea></td></tr>');  
			});  
