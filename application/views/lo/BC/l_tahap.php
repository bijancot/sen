	...
	<div class="card shadow show-grid">
		<div class="card-body">
			...
			$no = 1; foreach ($tahap_penilaian as $key) {
				...
				$hariini    	= date('d')."/".$bulan[date('m')]."/".date('Y');
				...
				//Convert Month
				$month_start	= $bulan[$pcs_start[1]];
				$month_end		= $bulan[$pcs_end[1]];

				//Calculate Month
				$date_start 	= $pcs_start[0]." ".$month_start." ".$pcs_start[2];
				$date_end	 	= $pcs_end[0]." ".$month_end." ".$pcs_end[2];

				if ($key->aktif == 0 ) {
                	$interval   = $time_start - $time_now;
					$sisa_hari	= strtotime($date_start)-strtotime($hariini);
				}elseif ($key->aktif == 1) {
                	$interval   = $time_end - $time_now;
					$sisa_hari	= strtotime($date_end)-strtotime($hariini);
				}else{
                	$interval   = $time_now-$time_now;
					$sisa_hari	= strtotime($hariini)-strtotime($hariini);
				}
					if ($sisa_hari > 0) {
						$sisa_waktu = $sisa_hari." Hari";
					}else{
						if ($interval > 0) {
				...
				<div class="tab" data-id="tab<?php echo $no;?>">
					<h5 class="mb-3 text-gray-800 mt-2">Detail <b>Tahap <?php echo $no;?> - <?php echo $key->nama_tahap;?></b> 
						<?php if ($key->aktif != 3) { ?>
						<button class="btn btn-danger btn-sm pull-right" data-toggle="modal" data-target="#hapus<?php echo $no; ?>">Hapus Tahap</button>
						<button class="btn btn-warning btn-sm pull-right mr-2" data-toggle="modal" data-target="#tunda<?php echo $no; ?>">TUNDA Tahap</button>
						<button class="btn btn-info btn-sm pull-right mr-2" data-toggle="modal" data-target="#edit<?php echo $no; ?>">Edit Tahap</button>
						<?php } ?></h5>
						<table class="table">
							<tr>
